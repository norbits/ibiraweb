...
function yx_cm(f){
    return f.document.doctype?true:false
    };
    
function yx_do(f){
    return f.document.body.offsetWidth
    };
    
function yx_dn(f){
    return f.document.body.offsetHeight
    };
...
function yx_eo(){
    ...
    if(si.fd.ax!=""){
        ...
        if(dg.bar){
            fh.style.styleFloat="left";
            fg.style.styleFloat="left"
            }
        }
};
...
this.ah=0;
if(gg!=yx_S){
    this.aw=Math.max(minimumWidth,yx_ei(this))+this.sw;
...
function yx_bg(f,cn){
    ...
    if(fu==null){
        if(typeof(f.yx_bm)=="undefined"||f.yx_bm==null){
            ...
            f.document.body.appendChild(f.yx_bm)
            };
